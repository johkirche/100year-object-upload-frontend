<script setup lang="ts">
    ...
    const findMatches = (options: Option[], parentPaths: string[] = []) => {
      for (const option of options) {
        ...
        if (option.children && option.children.length > 0) {
          findMatches(option.children, [...parentPaths, option.value])
        }
      }
    }
  ...
  const expandPathToItem = async (itemValue: string) => {
    ...
    const findItemParents = (options: Option[], currentPath: string[] = []): boolean => {
      for (const option of options) {
        ...
        if (option.children && option.children.length > 0) {
          const found = findItemParents(option.children, [...currentPath, option.value])
  ...
  const formattedCategories = computed(() => {
    ...
    return props.modelValue.map((cat: string) => categoryLookup[cat] || cat).join(', ')
  })
    ...
    <Popover v-model:open="popoverOpen">
      <PopoverTrigger as-child>
        <Button variant="outline" role="combobox" class="w-full justify-between" ref="triggerRef">
          <span v-if="selectedValues.length === 0" class="text-muted-foreground font-normal">
            {{ placeholder }}
          </span>
          <div v-else class="flex items-center gap-2 flex-grow min-w-0">
            ...
            <Badge v-if="showBadgeCount" variant="secondary" class="shrink-0"
              >{{ selectedValues.length }} ausgewählt</Badge
            >
          </div>
      ...
      <PopoverContent
        class="p-0 max-h-[500px] overflow-y-auto"
        :style="popoverStyle"
      ...
      >
        <Command class="w-full">
          ...
          <CommandGroup>
            <div
              class="flex flex-wrap gap-1 pb-2 pt-1 border-b mb-2"
            ...
            >
              ...
              >
                ...
                <button
                  @click.stop="removeItem(value, $event)"
            ...
            <TreeNode
              ...
              @update:selected="toggleSelection"
