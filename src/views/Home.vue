      ...
      <template v-if="currentStep === 'type-selection'">
        <div v-if="isLoading" class="flex justify-center mb-4">
          <div class="flex items-center">
            <div class="animate-spin h-5 w-5 mr-3 border-2 border-primary rounded-full border-t-transparent"></div>
        ...
        <div v-else-if="error" class="mb-4">
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Die aktuellen Informationen konnten nicht geladen werden. Bitte versuchen Sie es später erneut.
            </AlertDescription>
          </Alert>
        </div>
        ...
        <div v-else-if="statistics" class="mb-8 text-center">
          ...
          <p class="text-lg text-muted-foreground">
            Bisher wurden <span class="font-medium">{{ statistics.objectCount }}</span> Objekte hochgeladen und <span class="font-medium">{{ statistics.wishCount }}</span> Wünsche geäußert.
          </p>
        </div>
      </template>
...
async function fetchStatistics() {
  ...
  try {
    const response = await fetch(`${DIRECTUS_URL}/flows/trigger/12a60dff-bebc-4298-8a52-fe749f1e4999`)
    if (!response.ok) {
